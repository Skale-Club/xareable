...
- The planning call uses OpenRouter's real `json_schema` response format (strict structured output: `response_format: {type:"json_schema", json_schema:{name, strict:true, schema}}`), replacing today's loose `{ type: "json_object" }` + prompt-engineered field instructions. Strict mode requires every field in `required` (no true optionals — use nullable unions for genuinely optional fields) — this means the current `?`-heavy `GeminiCreativePlan`/`GeminiStructuredImagePrompt` interfaces need real restructuring, not a light edit.
- The direct-Gemini rollback path needs its own schema literal in Gemini's `responseSchema` dialect (uppercase `Type` enum strings like `"OBJECT"`/`"STRING"`) — this does NOT share OpenRouter's `json_schema` object; two separate schema literals, not one shared one.
- **Resolved ambiguity on schema-failure behavior:** existing retry-once-with-tightened-prompt behavior is UNCHANGED. If the retry ALSO fails schema validation, this is now a genuine planning failure — it surfaces as a real generation error to the user (same pattern already used by carousel's `CarouselTextPlanError`, which has no local-fallback template today), NOT a silently-degraded generic post. It is logged via a new `event_kind` in `generation_logs` (mirroring Phase 21's `model_fallback` pattern — `generation_logs.event_kind` is an unconstrained TEXT column, so this needs only a Zod enum widen, no migration). `buildLocalTextFallback`'s silent generic-template path is no longer reachable from a schema-validation failure.
- The existing transport-error fallback (network/auth failures — genuine connectivity issues, not schema/parse issues) remains unchanged and continues to use `buildLocalTextFallback` + the documented fallback path — this is the ONLY case that still uses it.
- Add a NEW dedicated `ai_models.planning` field (do NOT repoint the existing shared `ai_models.text_generation`, which 4 other call sites also use) for the planning call's admin-configurable model slug — confirmed via live OpenRouter capability check that none of the 3 Gemini slugs currently in the admin dropdown support `structured_outputs`; the executor must re-verify a real slug with confirmed `structured_outputs` support at implementation time (the catalog is dynamic) rather than hardcoding one from this research pass.
- `text_blocks` and `layout_archetype_id` are added to the `json_schema` as required fields with sensible defaults the model can safely emit today (top-level siblings of `headline`/`caption`; `layout_archetype_id` as an enum e.g. `["bottom_band","top_stack","centered_hero"]`), even though Phase 23's compositor is the only future consumer — so the schema never needs reopening.
