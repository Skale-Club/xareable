...
- **PLAN-07 scope — carousels included:** Style reference boards extend to BOTH single-image AND carousel generation. Research found carousels have NO reference-image mechanism at all today (a genuine gap) — since this phase's whole theme is bringing carousels to feature-parity with single-image (narrative structure, compositor, aesthetic DNA), leaving reference boards single-image-only would be an inconsistent, glaring omission. The same style-board attachment mechanism is reused for the carousel master-plan/slide generation calls.
- **`style_reference_photos` RLS:** follows the `app_settings` precedent exactly — genuine public `SELECT USING (true)` + admin-only write (`EXISTS (...profiles.is_admin = true)` subquery) — even though generation-time server reads will use the admin/service-role client regardless (bypassing RLS). This is for consistency and any future client-side read path, not just server-side necessity.
- **`/api/carousel/slide/edit` typography-awareness:** MUST be made typography-aware in THIS phase, not deferred. Since carousels now get real on-slide compositor text (CRSL2-02), an edit endpoint left in its old text-free assumption would immediately regress into the exact double-rendered/ghosted-text bug Phase 23 fixed for single images — shipping that would be a real, immediate regression, not an acceptable follow-up gap. Mirror Phase 23's `edit.routes.ts` base-image-aware edit pattern for the carousel slide-edit endpoint.
- **Carousel text-plan transport upgrade:** upgrade `callCarouselTextPlan` to strict `json_schema` (mirroring Phase 22's PLAN-02 pattern), rather than keeping the current loose `json_object` + manual validator. Since this phase already substantially reworks the carousel plan schema (adding `role`, `composition_note`, `text_blocks`, `layout_archetype_id`), doing the transport upgrade at the same time avoids reopening this file again later and gives the same reliability benefits (dependable structured JSON, no silent parse-failure fallback) Phase 22 gave the single-image path. Two separate schema dialects (OpenRouter vs. direct-Gemini) must not be cross-wired, per Phase 22's established precedent.
